...
public class Controller
{
 ...
 public MeetingPlaceArrayList run() throws IOException
 {
  ...
  Step[] step = parseSteps(this.address1,this.address2);
  Location loc = findMidPoint(step);
  MeetingPlaceArrayList meetList = findMeetingPlaces(loc, 2000);
  return meetList;
 }
 public Location findMidPoint(Step[] step) {
  ...
  // Finds the total distance of step from first to last, identifies what would be the middle distance, then finds the step that reaches that point.
   ...
   // Find Total Distance
   float totDist = 0; // the total distance tracker
   ...
   float midDist = totDist/2; // Finds half way distance. TO DO: IMPLEMENT RATIO
   
   // Find Mid Step
   float cumuDist = 0; // tracker for cumulative distance covered
   ...
   System.out.println("HERE: "+loc.getLat()+" + "+loc.getLng());
 ...
 public MeetingPlaceArrayList findMeetingPlaces(Location midpoint, int radius) throws IOException
 {
  ...
  for(int i = 0; i < splitData.length; i++)
  {
   ...
   if (splitData[i].contains("price_level"))
   {
    ...
    //Creates string to be displayed later to ensure data is correct and tre properly
